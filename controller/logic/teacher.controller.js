...
exports.createTeacher = (req, res, next) => {
    ...
    teacherDto.create(teacher, (err, data) => {
        ...
        userDto.create(user, (err, u) => {
            if (err) {
                teacherDto.deleteOne({ _id: data._id }, (e, data) => {
...
exports.updateTeacher = (req, res, next) => {
    ...
    teacherDto.updateOne({ _id: req.body.id }, teacher, (err, data) => {
        if (err) {
            return res.status(400).json(
                {
                    error: err
        ...
        if (req.body.olddocument != undefined) {
            ...
            userDto.updateOne({username: req.body.olddocument}, user, (err, u) => {
                if (err) {
                    return res.status(400).json(
                        {
                            error: err
